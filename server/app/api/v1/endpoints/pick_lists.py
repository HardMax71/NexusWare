...
@router.post("/", response_model=schemas.PickList)
...
@router.get("/", response_model=List[schemas.PickList])
def read_pick_lists(
        db: Session = Depends(deps.get_db),
        skip: int = 0,
        limit: int = 100,
...
@router.get("/optimize_route", response_model=schemas.OptimizedPickingRoute)
...
@router.get("/performance", response_model=schemas.PickingPerformance)
...
@router.get("/{pick_list_id}", response_model=schemas.PickList)
def read_pick_list(
...
@router.put("/{pick_list_id}", response_model=schemas.PickList)
...
@router.delete("/{pick_list_id}", response_model=schemas.PickList)
...
@router.post("/{pick_list_id}/start", response_model=schemas.PickList)
def start_pick_list(
        pick_list_id: int,
        db: Session = Depends(deps.get_db),
        current_user: models.User = Depends(deps.get_current_active_user)
...
@router.post("/{pick_list_id}/complete", response_model=schemas.PickList)
