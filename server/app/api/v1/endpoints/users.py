...
@router.post("/reset_password", response_model=schemas.Message)
def reset_password(
...
@router.get("/me", response_model=schemas.UserSanitizedWithRole)
def read_users_me(
    ...
    user = (db.query(models.User)
            ...
            .first())
    return schemas.UserSanitizedWithRole.model_validate(user)
...
@router.get("/", response_model=List[schemas.UserSanitizedWithRole])
def read_users(
        skip: int = 0,
        limit: int = 100,
        db: Session = Depends(deps.get_db),
    ...
    return [schemas.UserSanitizedWithRole.model_validate(user) for user in users]


@router.post("/", response_model=schemas.UserSanitizedWithRole)
def create_user(
        user: schemas.UserCreate,
        db: Session = Depends(deps.get_db),
        current_user: models.User = Depends(deps.get_current_admin)
    ...
    if db_user:
        raise HTTPException(status_code=400, detail="Email already registered")
    new_user = crud.user.create(db=db, obj_in=user)
    return schemas.UserSanitizedWithRole.model_validate(new_user)


@router.get("/{user_id}", response_model=schemas.UserSanitizedWithRole)
def read_user(
        user_id: int,
        db: Session = Depends(deps.get_db),
        current_user: models.User = Depends(deps.get_current_admin)
